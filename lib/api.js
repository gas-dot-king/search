...
import { normalizeRecoveryEvent, recoveryIsActive, recoveryNotice, recoveryState, RECOVERY_STATES } from "./recovery";
...
export async function requireUploadPeriod() {
  ...
  if (recoveryIsActive(event)) {
    throw new ApiError(recoveryNotice(event, recoveryState(event)), 423);
  }
  return settings;
}
...
export async function requireUserInRecoveryPeriod(req) {
  const [userResult, settingsResult] = await Promise.allSettled([requireUser(req), getSettings()]);
  if (userResult.status === "rejected") throw userResult.reason;
  if (settingsResult.status === "rejected") throw settingsResult.reason;
  const settings = settingsResult.value;
  const event = normalizeRecoveryEvent(settings.recovery_event);
  if (recoveryState(event) !== RECOVERY_STATES.ACTIVE) {
    throw new ApiError("긴급 복구 인증센터가 아직 열리지 않았거나 이미 닫혔어요.", 403);
  }
  return { user: userResult.value, settings, event };
}
