...
import { photoReview, takenBeforeEvent, takenBeforeToday } from "./photoReview";
 ...
 * sameDayOnly를 켜면 오늘(한국 시간) 찍은 사진만 받는다 — 빙고 인증 규칙이다.
 * 서버도 같은 검사를 하지만, 여기서 먼저 막아야 회원이 큰 사진을 올리고 나서 거절당하지 않는다.
 */
export function usePhoto({ uploadStart = null, sameDayOnly = false } = {}) {
  ...
  const pick = useCallback(async (e) => {
    ...
    try {
      ...
      const reason = takenBeforeEvent(photoMeta, uploadStart) ? "before_event"
        : sameDayOnly && takenBeforeToday(photoMeta) ? "before_today"
        : null;
      if (reason) {
        setRejected({
          reason,
