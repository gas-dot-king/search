...
import { photoReview, takenBeforeEvent } from "./photoReview";
 ...
 *
 * uploadStart를 주면 이벤트 시작 전에 찍은 사진을 받지 않는다. 서버도 같은 검사를 하지만,
 * 여기서 먼저 막아야 회원이 큰 사진을 올리고 나서 거절당하지 않는다.
 */
export function usePhoto({ uploadStart = null } = {}) {
  ...
  const pick = useCallback(async (e) => {
    ...
    try {
      ...
      // 이벤트 기간 밖에 찍은 사진은 인증이 될 수 없다. 올리기 전에 되돌려보낸다.
      if (takenBeforeEvent(photoMeta, uploadStart)) {
        setRejected({
