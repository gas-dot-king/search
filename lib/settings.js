...
import { LOTTO_ENTRY_LIMIT, currentLottoRound, parseLottoRounds } from "./lotto";
import { demoSettings, isDemoMode } from "./demo";
import { normalizeRecoveryEvent } from "./recovery";
...
export function publicConfig(settings) {
  return {
    ...
    // 1등이 없어 다시 뽑은 이력 — 번호가 왜 바뀌었는지 회원 화면에서 설명하는 데 쓴다
    ...
    recovery: normalizeRecoveryEvent(settings.recovery_event),
  };
}
