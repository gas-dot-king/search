...
export async function resizeForUpload(file, maxSide = 1200, quality = 0.8) {
  ...
  let meta = null;
  try {
    // JPEG EXIF APP1 세그먼트는 파일 앞부분에 있으므로 전체 원본(최대 30MB)을
    // 메모리에 한 번 더 복사하지 않고 앞 256KB만 읽는다.
    meta = readPhotoMetadata(new Uint8Array(await file.slice(0, 256 * 1024).arrayBuffer()));
  } catch {
    meta = null;
  }
...
export const CATEGORY_RULE =
  "하루에 각 카테고리에서 1개씩만 인증할 수 있어요. 하루 최대 3칸까지 가능해요.\n오늘이나 어제 찍은 사진만 인증할 수 있어요 — 그 이전에 찍은 사진은 올릴 때 자동으로 걸러집니다.\n빙고 4줄을 먼저 완성한 순서로 선착순 20명을 선정하며, 운영진이 인증 사진을 확인한 뒤 최종 결정합니다.";

// 업로드 모달에서 사진을 고르기 전에 알려 줄 규칙. 고르고 나서 거절당하면 헛일이 된다.
export const RECENT_PHOTO_RULE =
  "📅 빙고 인증은 오늘이나 어제 찍은 사진만 올릴 수 있어요. 그 이전에 찍은 사진은 자동으로 걸러집니다.";
