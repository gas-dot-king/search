...
export async function getUser(req) {
  ...
  const { data, error } = await sb()
    .from("users")
    .select("id, nickname, created_at, last_login_ip, token_expires_at")
  ...
  return { id: data.id, nickname: data.nickname, createdAt: data.created_at, lastLoginIp: data.last_login_ip };
}
