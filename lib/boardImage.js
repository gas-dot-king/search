...
export async function downloadBoardImage({ cells, filled, lines }) {
  if (document.fonts?.load) {
    await Promise.all([
      document.fonts.load("800 48px 'Noto Sans KR Variable'"),
      document.fonts.load("700 24px 'Noto Sans KR Variable'"),
    ]).catch(() => {});
  }
  ...
  if (!context) throw new Error("빙고판 이미지를 만들 수 없는 브라우저입니다.");
  context.fillStyle = "#faf9f7";
  context.fillRect(0, 0, canvas.width, canvas.height);
  context.fillStyle = "#e11d48";
  context.font = "800 48px 'Noto Sans KR Variable', sans-serif";
  context.fillText("YSRC SUMMER FEST 2026", padding, 68);
  context.fillStyle = "#475569";
  context.font = "600 28px 'Noto Sans KR Variable', sans-serif";
  ...
  for (const cell of cells) {
    ...
    context.strokeStyle = cell.hasPhoto || cell.photoUrl ? "#e11d48" : colors.border;
    context.lineWidth = cell.hasPhoto || cell.photoUrl ? 8 : 4;
    context.beginPath();
    context.roundRect(x, y, cellSize, cellSize, 22);
    context.fill();
    context.stroke();

    if (cell.hasPhoto || cell.photoUrl) {
      context.fillStyle = "#e11d48";
      context.beginPath();
    ...
    context.font = "700 24px 'Noto Sans KR Variable', sans-serif";
    context.textAlign = "center";
  ...
  const filename = "running-crew-bingo.png";
  const file = new File([blob], filename, { type: "image/png" });
  if (navigator.canShare?.({ files: [file] }) && navigator.share) {
    await navigator.share({ files: [file], title: "내 러닝 빙고판" });
    return;
  }
