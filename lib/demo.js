...
function createState() {
  return { users: [], cells: [], lotto: [], guestbook: [], settings: { ...DEMO_SETTINGS }, redraw: new Set() };
}
...
export function demoUpload(userId, position) {
  ...
  if (isNewToday) {
    ...
    if (submittedToday.length >= 3) return { error: "하루에는 빙고를 최대 3칸까지만 인증할 수 있어요." };
    const item = DEMO_ITEMS.find((entry) => entry.id === cell.item_id);
    const sameCategory = submittedToday.some((entry) =>
      DEMO_ITEMS.find((itemEntry) => itemEntry.id === entry.item_id)?.category === item?.category
    );
    if (sameCategory) return { error: "하루에는 같은 카테고리를 1칸만 인증할 수 있어요." };
  }
...
/** 관리자 검수용 4줄 선착순 (회원 id 포함) */
export function demoFourLineRanking() {
  const { users, cells } = demoProgress();
  const userOf = new Map(users.map((user) => [user.id, user]));
  return fourLineAchievements(cells).map(({ rank, userId, achievedAt }) => ({
    rank,
    id: userId,
    nickname: userOf.get(userId)?.nickname || "?",
    achievedAt,
  }));
}
...
export function demoGuestbookMine(userId) {
  const entry = state.guestbook.find((item) => item.user_id === userId);
  return entry ? guestbookView(entry) : null;
}

export function demoGuestbookAdd(userId, message) {
  if (state.guestbook.some((item) => item.user_id === userId)) {
    return { error: "이미 방명록을 남기셨어요. 기존 글을 수정해주세요.", status: 409 };
  }
