...
import { decodeRecentCursor, encodeRecentCursor } from "./adminCursor";
...
import { DEFAULT_RECOVERY_EVENT, normalizeRecoveryEvent, randomRecoveryTicket, recoveryTicketDigit, recoveryTicketLabel } from "./recovery";
...
const DEMO_SETTINGS = {
  ...
  recovery_event: JSON.stringify(DEFAULT_RECOVERY_EVENT),
};

function createState() {
  return {
    users: [], cells: [], lotto: [], guestbook: [], recovery: [], fourLineAwards: [], recoverySequence: 1000,
...
if (!state.recoverySequence) state.recoverySequence = 1000;
...
export function demoPublicConfig() {
  return {
    ...
    recovery: normalizeRecoveryEvent(state.settings.recovery_event),
  };
}

export function demoRecovery(userId) {
  const event = normalizeRecoveryEvent(state.settings.recovery_event);
  const mine = state.recovery.find((entry) => entry.user_id === userId && entry.event_key === event.key) || null;
  const winningDigit = event.winningDigit === "" ? null : Number(event.winningDigit);
  return {
    event,
    entry: mine ? {
      ticketNo: mine.ticket_no,
      ticket: recoveryTicketLabel(mine.ticket_no),
      digit: recoveryTicketDigit(mine.ticket_no),
      note: mine.note,
      createdAt: mine.created_at,
      mine: true,
    } : null,
    count: state.recovery.filter((entry) => entry.event_key === event.key).length,
    winningDigit,
    winners: winningDigit === null ? [] : state.recovery
      .filter((entry) => entry.event_key === event.key && recoveryTicketDigit(entry.ticket_no) === winningDigit)
      .map((entry) => ({ nickname: state.users.find((user) => user.id === entry.user_id)?.nickname || "?", ticket: recoveryTicketLabel(entry.ticket_no) })),
  };
}

export function demoRecoveryAdd(userId, note = "") {
  const event = normalizeRecoveryEvent(state.settings.recovery_event);
  const exists = state.recovery.find((entry) => entry.user_id === userId && entry.event_key === event.key);
  if (exists) return { error: "이미 긴급 복구 인증을 제출했어요.", status: 409 };
  const ticketNo = randomRecoveryTicket();
  const entry = { ticket_no: ticketNo, event_key: event.key, user_id: userId, note: String(note || "").slice(0, 120), created_at: now() };
  state.recovery.push(entry);
  return { ok: true, entry: { ticketNo, ticket: recoveryTicketLabel(ticketNo), digit: recoveryTicketDigit(ticketNo), note: entry.note, createdAt: entry.created_at, mine: true } };
}

export function demoRecoveryAdmin() {
  const event = normalizeRecoveryEvent(state.settings.recovery_event);
  const entries = state.recovery
    .filter((entry) => entry.event_key === event.key)
    .sort((a, b) => a.created_at.localeCompare(b.created_at))
    .map((entry) => ({
      ticketNo: entry.ticket_no,
      ticket: recoveryTicketLabel(entry.ticket_no),
      digit: recoveryTicketDigit(entry.ticket_no),
      userId: entry.user_id,
      nickname: state.users.find((user) => user.id === entry.user_id)?.nickname || "?",
      note: entry.note,
...
export function demoDrawRecoveryDigit() {
  const event = normalizeRecoveryEvent(state.settings.recovery_event);
  if (event.winningDigit !== "") return { ok: true, digit: Number(event.winningDigit), alreadyDrawn: true };
  const entries = state.recovery.filter((entry) => entry.event_key === event.key);
  if (!entries.length) return { error: "아직 복구 인증 접수가 없습니다.", status: 409 };
  const digits = [...new Set(entries.map((entry) => recoveryTicketDigit(entry.ticket_no)))];
  const digit = digits[Math.floor(Math.random() * digits.length)];
  state.settings.recovery_event = JSON.stringify({ ...event, winningDigit: String(digit) });
  return {
    ok: true,
    digit,
    winners: entries.filter((entry) => recoveryTicketDigit(entry.ticket_no) === digit)
      .map((entry) => ({ nickname: state.users.find((user) => user.id === entry.user_id)?.nickname || "?", ticket: recoveryTicketLabel(entry.ticket_no) })),
  };
}
...
export function demoRecentUploads(before, pageSize) {
  ...
  const rows = state.cells
    ...
    .filter((cell) => !cursor
      || cell.uploaded_at < cursor.at
      || (cell.uploaded_at === cursor.at && cursor.id && String(cell.id) < cursor.id))
    .sort((a, b) => b.uploaded_at.localeCompare(a.uploaded_at) || String(b.id).localeCompare(String(a.id)));
  ...
  return {
    ...
    nextCursor: rows.length > pageSize ? encodeRecentCursor(page[page.length - 1]) : null,
  };
}
