...
/**
 * now가 속한 한국 날짜의 자정(ms). daysBack만큼 앞선 날의 자정도 구할 수 있다.
 * (daysBack=1이면 어제 자정)
 */
export function startOfSeoulDay(now = new Date(), daysBack = 0) {
  const kst = now.getTime() + KST_OFFSET_MS;
  return (Math.floor(kst / DAY_MS) - daysBack) * DAY_MS - KST_OFFSET_MS;
}

/** 빙고 인증으로 받아 주는 촬영 날짜 범위 — 오늘과 어제(한국 시간) */
export const RECENT_PHOTO_DAYS = 2;

/**
 * 빙고 인증에 쓰기엔 너무 오래된 사진인지 — 오늘·어제(한국 시간) 찍은 것만 받는다.
 *
 * 며칠 전에 찍어 둔 사진을 몰아서 올리는 걸 막는 규칙이다. 어제까지 열어 두는 건
 * 새벽에 달리고 다음 날 올리거나, 자정 무렵에 올리다 날짜가 넘어가는 경우를 위해서다.
 *
 ...
 */
export function takenBeforeRecentDays(meta, now = new Date()) {
  const takenMs = takenAtMs(meta);
  if (takenMs === null) return false;
  return takenMs < startOfSeoulDay(now, RECENT_PHOTO_DAYS - 1);
}
