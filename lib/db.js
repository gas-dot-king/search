...
// 그리드용 축소본은 원본 경로에서 규칙으로 유도한다. 컬럼을 늘리지 않아도 되고,
// 축소본이 없던 시절에 올린 사진은 서명이 실패하면서 자연히 원본으로 되돌아간다.
export function thumbPathFor(path) {
  if (!path) return null;
  // 이미 축소본 경로면 그대로 둔다 — 정리 목록을 만들 때 두 번 씌워도 안전하도록.
  if (/\.thumb\.jpg$/i.test(path)) return path;
  return path.replace(/\.jpg$/i, ".thumb.jpg");
}

// PostgREST는 한 요청에 최대 1000행만 돌려준다. cells는 회원 수 × 16으로 늘어나
// 이 상한을 넘는 순간 "조용히 잘린 목록"이 되고, 그걸로 계산한 진행률·4줄 순위가
// 전부 틀어진다. 오류도 안 나므로 눈치채기 어렵다. 나눠 읽어 그 사고를 막는다.
const SELECT_PAGE_SIZE = 1000;

/**
 * 상한을 넘는 표를 페이지로 나눠 전부 읽는다.
 * refine으로 필터·정렬을 붙이되, 페이지 사이에 행이 겹치거나 빠지지 않도록
 * 반드시 안정적인 정렬(고유 컬럼)을 포함해야 한다.
 */
export async function selectAllRows(table, columns, refine = (query) => query) {
  const rows = [];
  for (let from = 0; ; from += SELECT_PAGE_SIZE) {
    const { data, error } = await refine(sb().from(table).select(columns))
...
export async function processPhotoCleanup(paths = null) {
  ...
  try {
    ...
    // attempts를 그냥 1로 쓰면 몇 번째 실패인지 알 수 없다. DB에서 증가시켜야
    // "일시적 오류"와 "영영 안 지워지는 파일"을 구분할 수 있다.
    const { error: recordError } = await sb()
      .rpc("record_cleanup_failure", { p_paths: valid, p_error: message });
...
/** 여러 번 실패한 파일은 일시적 오류가 아니라 손봐야 할 문제로 본다 */
const CLEANUP_STUCK_ATTEMPTS = 3;

/** 밀린 정리 작업 현황 — 관리자 화면에서 "고아 파일이 쌓이고 있는지" 보는 용도 */
export async function photoCleanupStatus() {
  const [{ count, error: countError }, { count: stuck, error: stuckError }, { data, error }] =
    await Promise.all([
      sb().from("storage_cleanup_tasks").select("path", { count: "exact", head: true }),
      sb()
        .from("storage_cleanup_tasks")
        .select("path", { count: "exact", head: true })
        .gte("attempts", CLEANUP_STUCK_ATTEMPTS),
      sb()
        .from("storage_cleanup_tasks")
        .select("path, created_at, attempts, last_error")
        .order("attempts", { ascending: false })
        .order("created_at")
        .limit(5),
    ]);
  const failure = countError || stuckError || error;
  if (failure) throw new Error(`사진 정리 현황을 읽지 못했습니다: ${failure.message}`);

  const rows = data || [];
  return {
    pending: count || 0,
    stuck: stuck || 0,
    oldest: rows.length ? rows.map((row) => row.created_at).sort()[0] : null,
    samples: rows.map((row) => ({
      path: row.path,
      attempts: row.attempts,
      lastError: row.last_error ? String(row.last_error).slice(0, 120) : null,
    })),
  };
}
...
/**
 * 여러 사진의 서명 URL을 한 번에 만든다.
 * retryMissing=false면 일괄 서명에서 빠진 경로를 개별로 다시 시도하지 않는다.
 * 축소본처럼 "없을 수도 있는" 경로에 쓴다 — 없는 파일마다 왕복을 더하면 오히려 느려진다.
 */
export async function signedUrls(paths, { retryMissing = true } = {}) {
  const valid = [...new Set(paths.filter(Boolean))];
  if (valid.length === 0) return {};
  const map = {};
  const missing = [];
  ...
  if (error) {
    if (!retryMissing) return map;
    throw new Error(`사진 주소를 만들지 못했습니다: ${error.message}`);
  }
