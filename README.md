...
3. 이어서 `supabase/migrations/`의 SQL 파일을 파일명 순서대로 실행합니다. 기존 운영 DB는 아직 적용하지 않은 migration만 실행하세요. 특히 `20260730000000_harden_accounts_and_bingo.sql`은 PIN 잠금, 세션 해시, 일일 빙고 제한과 원자적 재추첨을 추가하므로 **앱 배포 전에 반드시 적용**해야 합니다.
   ...
   - `20260803000000_add_recovery_event.sql`은 8/4 긴급 복구 인증과 접수번호를 추가하므로 이벤트 배포 전에 적용하세요.
...
- **관리자 페이지**: `/admin` (URL 직접 입력) → `ADMIN_PASSWORD` 로 입장
  ...
  - **긴급 복구 이벤트**: 8/3 18:00부터 안내 팝업, 8/4 00:00~8/5 00:00 동안 일반 빙고·로또 인증을 잠그고 `/recovery`에서 1인 1회 복구 인증을 받습니다. 접수번호 끝자리 숫자를 추첨하며 기존 진행도에는 반영하지 않습니다.
  - 복구 시간이 끝나면 일반 인증은 자동으로 재개됩니다. 관리자 화면의 긴급 복구 카드에서 접수 현황을 보고 숫자를 추첨하세요.
  - **8/15 추첨**: 관리자 화면의 "무작위 번호 추첨 (큰 화면)" 버튼 → `/admin/draw`에서 자리별로 릴을 돌려 뽑습니다. 뽑은 숫자는 즉시 저장되어 회원 로또 탭에도 공개됩니다
    - 세 자리가 다 나오면 1등이 있는지 자동으로 확인합니다. **1등이 없으면 "N차 추첨 시작하기"** 로 다음 차수를 시작하고, 직전 번호는 "1등 없음" 기록으로 남아 회원 화면에도 표시됩니다
    - 1등이 나온 차수는 다음 차수로 넘어갈 수 없습니다. 차수 기록까지 전부 지우려면 "처음부터 다시"를 쓰세요
