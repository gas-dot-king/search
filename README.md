...
- **관리자 페이지**: `/admin` (URL 직접 입력) → `ADMIN_PASSWORD` 로 입장
  ...
  - **8/15 추첨**: "당첨 번호 추첨하기" 버튼 → 자리별 0~9 랜덤. 추첨 즉시 모든 회원의 로또 탭이 결과 화면으로 바뀜
    - 행사장에서는 `/admin/draw`(관리자 화면의 "무작위 번호 추첨" 버튼)를 띄워 한 자리씩 릴을 돌립니다. 뽑는 동작은 위 버튼과 같아 결과가 그대로 저장됩니다.
  - **4줄 달성 선착순**: 4번째 줄을 채운 인증 시각 순으로 자동 정렬됩니다. 선착순 20명이 선물 대상이며, "보기"로 인증 사진을 확인한 뒤 확정하세요
