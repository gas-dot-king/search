...
describe("최근 촬영 판정 (빙고 인증 — 오늘·어제만)", () => {
  // 한국 시간 8월 4일 오전 10시
  ...
  it("한국 시간 자정을 기준선으로 잡는다", () => {
    ...
    expect(new Date(startOfSeoulDay(NOW, 1)).toISOString()).toBe("2026-08-02T15:00:00.000Z");
  });
  ...
  it("오늘 찍은 사진은 통과시킨다", () => {
    expect(takenBeforeRecentDays({ takenAt: "2026-08-04T00:00:00" }, NOW)).toBe(false);
    expect(takenBeforeRecentDays({ takenAt: "2026-08-04T09:59:00" }, NOW)).toBe(false);
  });

  it("어제 찍은 사진도 통과시킨다", () => {
    // 새벽에 달리고 다음 날 올리는 경우가 있어 어제까지는 열어 둔다.
    expect(takenBeforeRecentDays({ takenAt: "2026-08-03T00:00:00" }, NOW)).toBe(false);
    expect(takenBeforeRecentDays({ takenAt: "2026-08-03T23:59:59" }, NOW)).toBe(false);
  });

  it("그저께 이전 사진은 막는다", () => {
    expect(takenBeforeRecentDays({ takenAt: "2026-08-02T23:59:59" }, NOW)).toBe(true);
    expect(takenBeforeRecentDays({ takenAt: "2026-07-30T07:00:00" }, NOW)).toBe(true);
  });

  it("기기 시계가 앞서 미래로 찍힌 사진은 막지 않는다", () => {
    // 몇 분 빠른 시계 때문에 정상 인증이 거절되는 쪽이 더 나쁘다.
    expect(takenBeforeRecentDays({ takenAt: "2026-08-05T23:00:00" }, NOW)).toBe(false);
  });

  it("촬영 정보가 없으면 막지 않는다", () => {
    // 러닝 앱 화면 캡처에는 EXIF가 없다. 막으면 기록 달성 칸을 아무도 못 채운다.
    expect(takenBeforeRecentDays(null, NOW)).toBe(false);
    expect(takenBeforeRecentDays({}, NOW)).toBe(false);
    expect(takenBeforeRecentDays({ make: "Apple" }, NOW)).toBe(false);
  });

  it("기기가 남긴 시간대를 반영해서 판정한다", () => {
    // UTC로 8/2 20:00에 찍힌 사진은 한국 시간 8/3 05:00이라 어제다.
    expect(takenBeforeRecentDays({ takenAt: "2026-08-02T20:00:00", utcOffset: "+00:00" }, NOW)).toBe(false);
    // 같은 벽시계 값이라도 한국에서 찍혔다면 그저께다.
    expect(takenBeforeRecentDays({ takenAt: "2026-08-02T20:00:00", utcOffset: "+09:00" }, NOW)).toBe(true);
  });
});
